...
export class PaymentPageController {
  constructor(
    ...
    private readonly getPaymentCheckoutService: GetPaymentCheckoutService,
  ) {}
  ...
  selectBid = async (
    ...
    try {
      ...
      if (typeof visitId !== "string" || typeof bidId !== "string") {
        response.status(404).send("Visit or bid not found");
      ...
      response.redirect(303, `/mock-payments/${payment.id}`);
    } catch (error) {
      next(error);
    }
  };
  ...
  showCheckout = async (
    ...
    try {
      ...
      const payment = await this.getPaymentCheckoutService.execute({
        paymentId,
        patientId,
      });

      response.status(200).render("patient/mock-checkout", {
        title: "Complete payment",
        payment: {
          id: payment.id,
          visitId: payment.visitId,
          reference: payment.providerReference,
          amount: currencyFormatter.format(payment.amountInKobo / 100),
          status: payment.status,
        },
        isPending: payment.status === "PENDING",
        isSucceeded: payment.status === "SUCCEEDED",
