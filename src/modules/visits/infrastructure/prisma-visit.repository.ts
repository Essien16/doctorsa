...
async listOpenForSpecialty(
  ...
  return visits.map((visit) => {
    if (
      ...
      throw new Error(
        `Unexpected open visit status: ${visit.status}`,
      );
    }
...
async submitBid(
  ...
  return this.prisma.$transaction(
    async (transaction): Promise<SubmitBidResult> => {
      ...
      const doctorProfile =
        await transaction.doctorProfile.findUnique({
          where: {
  ...
  return {
    ...
    selectedBidId: visit.selectedBidId,
    assignedDoctorName:
      visit.assignedDoctor?.user.name ?? null,
...
async listAssignedForDoctorProfile(
  ...
  return visits.map((visit) => {
    if (!visit.selectedBid) {
      throw new Error(
        `Assigned visit ${visit.id} has no selected bid`,
      );
    }
