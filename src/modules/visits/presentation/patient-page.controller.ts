...
export class PatientPageController {
  constructor(
    private readonly listPatientVisitsService: ListPatientVisitsService,
  ...
  showVisits = async (
    ...
    try {
      ...
      const visits = await this.listPatientVisitsService.execute(patientId);
  ...
  createVisit = async (
    ...
    try {
      ...
      const validation = createVisitPageSchema.safeParse(request.body);

      if (!validation.success) {
        await this.renderCreateVisitPage(response, 400, {
          errorMessage: "Please correct the highlighted information.",
          values: request.body,
       ...
       */
      const preferredAt = new Date(`${validation.data.preferredAt}:00+01:00`);

      if (Number.isNaN(preferredAt.getTime()) || preferredAt <= new Date()) {
        await this.renderCreateVisitPage(response, 400, {
          errorMessage: "Preferred time must be in the future.",
          values: validation.data,
          fieldErrors: {
            preferredAt: ["Select a date and time in the future."],
          },
        });
      ...
      try {
        ...
        if (error instanceof ConflictError) {
          await this.renderCreateVisitPage(response, 409, {
            errorMessage:
  ...
  showVisitDetails = async (
    ...
    try {
      ...
      if (typeof visitId !== "string" || visitId.trim().length === 0) {
      ...
      response.status(200).render("patient/visit-details", {
        ...
        bids: visit.bids.map((bid) => ({
          ...
          amount: currencyFormatter.format(bid.amountInKobo / 100),
          note: bid.note,
          submittedAt: dateFormatter.format(bid.createdAt),
          selected: visit.selectedBidId === bid.id,
          canSelect: visit.status === "BIDDING" && visit.selectedBidId === null,
        })),
      });
    } catch (error) {
      next(error);
  ...
  private async renderCreateVisitPage(
    ...
    const specialties = await this.listSpecialtiesService.execute();

    response.status(status).render("patient/new-visit", {
      title: "Request a visit",
      errorMessage: data.errorMessage,
      location: data.values?.location ?? "",
      preferredAt: data.values?.preferredAt ?? "",
      specialtyError: data.fieldErrors?.specialtyId?.[0],
      locationError: data.fieldErrors?.location?.[0],
      preferredAtError: data.fieldErrors?.preferredAt?.[0],
      specialties: specialties.map((specialty) => ({
        id: specialty.id,
        name: specialty.name,
        selected: specialty.id === data.values?.specialtyId,
      })),
    });
  }
}
